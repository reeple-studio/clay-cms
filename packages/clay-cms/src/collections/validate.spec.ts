...
describe("validateCollections — slug + field-name format (July 2026 review)", () => {
	it("rejects a slug that isn't a safe identifier", () => {
		// ? leading digit would break the generated TS interface name
		expect(() =>
			validateCollections(
				[users, { ...posts, slug: "2fa" }],
				undefined,
				adminCfg,
			),
		).toThrow(/Invalid collection slug "2fa"/);
	});

	it("accepts a kebab-case slug (quoted safely downstream)", () => {
		expect(() =>
			validateCollections(
				[users, { ...posts, slug: "blog-posts" }],
				undefined,
				adminCfg,
			),
		).not.toThrow();
	...
	it("rejects two slugs that collapse to the same TS type name", () => {
		// ? post + posts both → `Post`
		expect(() =>
			validateCollections(
				[users, { ...posts, slug: "post" }, { ...posts, slug: "posts" }],
				undefined,
				adminCfg,
			),
		).toThrow(/both generate the TypeScript type name "Post"/);
	});
		...
		expect(() =>
			validateCollections(
				[
					...
					{
						slug: "widgets",
						fields: { "bad name": { type: "text" } },
					},
				],
				undefined,
				adminCfg,
			),
		).toThrow(/Invalid field name "bad name"/);
	});

	it("rejects a user field that collides with a reserved auth field", () => {
		expect(() =>
			validateCollections(
				[
					{
		...
		).toThrow(/"hashedPassword".*collides with a reserved field/);
	});

	it("rejects a user field colliding with a system field", () => {
		expect(() =>
			validateCollections(
				[users, { slug: "posts", fields: { id: { type: "text" } } }],
				undefined,
				adminCfg,
			),
		).toThrow(/"id".*collides with a reserved field/);
	});
});
