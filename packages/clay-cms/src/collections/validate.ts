import { docTypeName } from "../handlers/types.js";
import type { AdminConfig } from "../types.js";
import type { CollectionConfig, LocalizationConfig } from "./types.js";

// ? slug: must be a safe identifier for the SQL table name, the cms proxy key,
// ? and (via docTypeName) the generated TS interface name.
const SLUG_RE = /^[a-z][a-z0-9_-]*$/;
// ? field name: becomes a DB column + a TS property. Leading `_` is reserved for
// ? Clay's internal columns (`_parentId`, `_locale`), so disallow it.
const FIELD_NAME_RE = /^[A-Za-z][A-Za-z0-9_-]*$/;

const SYSTEM_RESERVED = ["id", "createdAt", "updatedAt"];
const UPLOAD_RESERVED = [
	"filename",
	"mimeType",
	"filesize",
	"url",
...
const AUTH_RESERVED = ["email", "hashedPassword"];
...
export function validateCollections(
	...
	// ? slug format — reject anything that would break the generated .d.ts, the
	// ? SQL table name, or the cms proxy key (leading digits, spaces, quotes…).
	for (const c of collections) {
		if (!SLUG_RE.test(c.slug)) {
			throw new Error(
				`[clay-cms] Invalid collection slug "${c.slug}". Slugs must start with a lowercase letter and contain only lowercase letters, digits, "-", or "_".`,
			);
		}
	}

	// ? derived TS type-name collisions — two distinct slugs can singularize +
	// ? PascalCase to the same interface name (post/posts → Post), producing a
	// ? "Duplicate identifier" error in the injected types.
	const derivedNames = new Map<string, string>();
	for (const c of collections) {
		const derived = docTypeName(c.slug);
		const prior = derivedNames.get(derived);
		if (prior) {
			throw new Error(
				`[clay-cms] Collections "${prior}" and "${c.slug}" both generate the TypeScript type name "${derived}". Rename one slug.`,
			);
		}
		derivedNames.set(derived, c.slug);
	}

	// ? field-name format + reserved-name collisions. Reserved system/upload/auth
	// ? fields are managed by Clay (resolveCollections auto-merges them); a user
	// ? field of the same name would otherwise silently shadow flags like
	// ? hashedPassword's `hidden` (leaking hashes) — resolve.ts now spreads
	// ? reserved fields last as defense in depth, but reject it loudly here.
	for (const c of collections) {
		const reserved = new Set<string>([
			...SYSTEM_RESERVED,
			...(c.upload ? UPLOAD_RESERVED : []),
			...(c.auth ? AUTH_RESERVED : []),
		]);

		for (const key of Object.keys(c.fields)) {
			if (!FIELD_NAME_RE.test(key)) {
				throw new Error(
					`[clay-cms] Invalid field name "${key}" in collection "${c.slug}". Field names must start with a letter and contain only letters, digits, "-", or "_" (no leading "_").`,
				);
			}

			if (reserved.has(key)) {
				throw new Error(
					`[clay-cms] Field "${key}" in collection "${c.slug}" collides with a reserved field Clay manages automatically. Remove it.`,
				);
			}
		}
	}
