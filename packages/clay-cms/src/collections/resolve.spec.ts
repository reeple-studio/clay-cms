...
describe("resolveCollections — reserved field protection (July 2026 review)", () => {
	it("reserved auth fields win over a user field of the same name", () => {
		// ? even if a user declares `hashedPassword` without `hidden`, the merge
		// ? must keep Clay's hidden hashedPassword so hashes never render.
		const [resolved] = resolveCollections([
			{
				...
				fields: {
					...
					hashedPassword: { type: "text" }, // ? no `hidden` — a footgun
				},
			},
		]);
		...
		const hp = resolved.fields.hashedPassword;
		expect(hp).toBeDefined();
		expect("hidden" in hp && hp.hidden).toBe(true);
	});

	it("system fields win over a user field of the same name", () => {
		const [resolved] = resolveCollections([
			{ slug: "posts", fields: { id: { type: "number" } } },
		]);

		if (!resolved) throw new Error("expected resolved collection");
		// ? system `id` is text+required, not the user's number field
		expect(resolved.fields.id.type).toBe("text");
	});
});
