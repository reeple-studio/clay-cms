...
export function resolveCollections(
	...
	return collections.map((collection) => {
		// ? Reserved system/upload/auth fields are spread AFTER the user's fields so
		// ? they always win — a user cannot redefine `hashedPassword` (and strip its
		// ? `hidden` flag, leaking hashes), `id`, `createdAt`, or an upload field.
		// ? validateCollections() additionally rejects such collisions at boot with a
		// ? clear error; this ordering is the runtime defense in depth.
