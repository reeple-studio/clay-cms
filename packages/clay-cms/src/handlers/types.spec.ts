...
describe("buildTypesContent — interface naming", () => {
	...
	it("multi-word kebab slug: blog-posts → BlogPost", () => {
		...
		// ? the CMS-map key MUST be quoted — a bare `blog-posts:` is a TS syntax
		// ? error that breaks the whole generated .d.ts. (Regression: this test
		// ? previously green-locked the unquoted, invalid output.)
		expect(out).toContain('"blog-posts": CollectionAPI<BlogPost,');
		expect(out).not.toMatch(/[^"]blog-posts: CollectionAPI/);
	});
});
...
describe("buildTypesContent — auth collection types", () => {
	it("includes auto-merged email + hashedPassword fields on auth interfaces", () => {
		...
		// ? hashedPassword is hidden → optional in the Doc type (omitted from
		// ? find/findOne results unless showHiddenFields under overrideAccess).
