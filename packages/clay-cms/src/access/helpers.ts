...
// ? not — boolean only. Negating a Where is ill-defined (would need a NOT operator
// ? in the type), so we refuse it at runtime rather than silently coerce — a
// ? Where-returning fn wrapped in not() almost always means "allowed for matching
// ? docs", and flipping that to `false` would over-deny without the author noticing.
...
export function not(fn: AccessFn): AccessFn {
	return async (ctx: AccessContext) => {
		...
		if (typeof result !== "boolean") {
			throw new Error(
				"[clay-cms] not() received a Where-returning access fn — only boolean-returning fns can be negated.",
			);
		}
		return !result;
	};
