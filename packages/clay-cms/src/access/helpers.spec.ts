...
import type { AccessContext, AccessFn } from "./types.js";
...
describe("not", () => {
	...
	it("throws when wrapping a Where-returning access fn (ill-defined negation)", async () => {
		const whereFn: AccessFn = () => ({ author: { equals: "u-1" } });

		await expect(not(whereFn)(ctx())).rejects.toThrow(
			/only boolean-returning fns can be negated/,
		);
	});
});
