...
describe("matchesWhere — SQL-evaluator parity (July 2026 review)", () => {
	// ? these pin the semantics whereToDrizzle was aligned to, so the two
	// ? evaluators can't drift (find() vs the findOne/update/delete gate).
	it("not_equals matches a NULL/missing field (NULL-inclusive)", () => {
		expect(matchesWhere({ role: { not_equals: "admin" } }, { id: "1" })).toBe(
			true,
		);
		expect(
			matchesWhere({ role: { not_equals: "admin" } }, { id: "1", role: null }),
		).toBe(true);
	});

	it("not_in matches a NULL/missing field (NULL-inclusive)", () => {
		expect(
			matchesWhere({ role: { not_in: ["admin", "editor"] } }, { id: "1" }),
		).toBe(true);
	});

	it("non-array `in` fails closed (matches nothing)", () => {
		// ? mirrors whereToDrizzle emitting `1 = 0` for a non-array operand
		expect(
			matchesWhere({ x: { in: "oops" as unknown as string[] } }, { x: "oops" }),
		).toBe(false);
	});

	it("non-array `not_in` fails closed (matches nothing)", () => {
		expect(
			matchesWhere(
				{ x: { not_in: "oops" as unknown as string[] } },
				{ x: "y" },
			),
		).toBe(false);
	});
