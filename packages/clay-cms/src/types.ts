...
// ? real→number (float), boolean→boolean, timestamp→ISO-8601 string,
// ? json→parsed JS value.
export interface ColumnBuilders {
	...
	// ? floating-point number. `integer` is for ids/counters/whole numbers;
	// ? `real` preserves fractional values (price, rating). D1/SQLite → REAL.
	// biome-ignore lint/suspicious/noExplicitAny: dialect-abstraction boundary
...
export interface DatabaseAdapterResult {
	...
	// ? Legacy imperative adapter surface. The runtime CRUD path goes through the
	// ? `drizzle` accessor (runtime/api.ts), so this is unused today; kept optional
	// ? for adapters that want to expose an imperative handle. See DatabaseAdapter.
	init?: (
		collections: ResolvedCollectionConfig[],
		localization?: LocalizationConfig,
	) => DatabaseAdapter;
