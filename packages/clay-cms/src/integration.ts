...
export const clay = (opts: ClayIntegrationOptions = {}): AstroIntegration => {
	// ? shared closure state populated in astro:config:setup and reused by
	// ? config:done + server:setup. Hooks always run in this order.
	...
	return {
		...
		hooks: {
			...
			"astro:config:done": (params) => {
				injectClayTypes(params, resolved, userConfig.localization);
			},
			"astro:server:setup": () => {
