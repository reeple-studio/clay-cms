...
export function r2(config: R2AdapterConfig): StorageAdapterResult {
	return {
		...
		init: (): StorageAdapter => {
			...
			// ? single source of truth for the public URL shape so handleUpload and
			// ? generateUrl can't disagree (one returned "file.jpg", the other
			// ? "/file.jpg" → broken/relative links for the first caller to wire this).
			const toUrl = (p: string): string => `/${p}`;
