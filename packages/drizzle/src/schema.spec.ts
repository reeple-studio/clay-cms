...
import { sqliteSchemaConfig } from "./sqlite.js";

// ? exercise the SAME shared SQLite vocabulary the adapters ship, so these
// ? assertions actually pin the lowering the runtime uses.
const sqliteConfig = sqliteSchemaConfig;
...
describe("buildSchema", () => {
	...
	it("maps field types to correct SQL types", () => {
		...
		expect(byName.views.getSQLType()).toBe("real"); // ? number → real (float-safe)
