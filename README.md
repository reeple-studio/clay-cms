...
Need only a few fields? Pass `select` to project the row down to what you'll
actually render — narrows the SQL on the wire **and** the TypeScript return
type. Include mode (`{ field: true }`) returns just the listed fields plus
`id`/`createdAt`/`updatedAt`; exclude mode (`{ field: false }`) drops the
listed fields and keeps everything else.

```ts
// only the columns the listing actually shows
const orders = await cms.orders.find({
  user: Astro.locals.clayUser,
  select: { total: true, status: true },
});
// orders is typed as Array<Pick<Order, "id" | "createdAt" | "updatedAt" | "total" | "status">>
```
...
playground/             ← Astro 7 + Cloudflare demo
```
...
- Florian Lefebvre for the [Astro Integration Kit](https://github.com/florian-lefebvre/astro-integration-kit), which Clay's integration was originally built on.
